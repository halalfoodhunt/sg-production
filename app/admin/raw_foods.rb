ActiveAdmin.register RawFood do
    ...
    form do |f|
        ...
        f.input  "raw_food_product_categories"
        f.input  "slug"
        f.inputs do
  ...
  csv do
    column  "id"
    column("Merchant"){|raw_food| raw_food.merchant.name }
    column("Listing"){|raw_food| raw_food.listing.name }
    column "brand_name"
    column "general_email"
    ...
    column "halal_expiry"
    column "expiry_date"
    column("product_qualifying_type"){|raw_food| raw_food.product_qualifying_type.name }
    column "slug"
  end
