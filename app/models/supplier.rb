class Supplier < ActiveRecord::Base
  ...
  has_many :opening_hours, dependent: :destroy, inverse_of: :supplier
  accepts_nested_attributes_for :opening_hours, allow_destroy: true, reject_if: ->(attrs) { attrs['day_id'].blank? }
  
  has_many :menus, dependent: :destroy, inverse_of: :supplier
  accepts_nested_attributes_for :menus, reject_if: :all_blank, allow_destroy: true
  
  has_many :verifying_documents, dependent: :destroy, inverse_of: :supplier
